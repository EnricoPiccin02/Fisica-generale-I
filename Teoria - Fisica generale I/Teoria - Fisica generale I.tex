...
\begin{tabularx}{\textwidth}{@{}|P|@{}}
    ...
    Analogamente si può interpretare il prodotto scalare tra due vettori $v$ e $u$ come il prodotto dei moduli per il \textbf{coseno} dell'angolo $\theta$ compreso tra i vettori stessi, ovvero
    \[\vec{v} \cdot \vec{u} = v \cdot c \cdot \cos(\theta)\]
    \vspace{-1mm}}\\
    \hline
\end{tabularx}
...
\begin{itemize}
  ...
  \item \(\hat{i} \cdot \hat{j} = \hat{i} \cdot \hat{k} = \hat{j} \cdot \hat{k} = 0\). Questo significa che i due vettori considerati sono ortogonali.
\end{itemize}
...
e questo significa che $\vec{v} \cdot \hat{i}$ è la proiezione del vettore $\vec{v}$ in direzione $\hat{i}$.\\
Per esempio, se si considerando due vettori $\vec{c}$ e $\vec{d}$, allora il loro prodotto scalare può essere interpretato come segue
