...
Nel caso in cui le due molle da considerare siano in parallelo, allora è come se si considerasse una molla con costante di elasitcià doppia; a partire dall'equazione seguente
...
\[\vec{r_{\text{CM}}} = \frac{\displaystyle{\sum_{i} \vec r_i \cdot m_1}}{M}\]
...
\[\vec a_{\text{CM}} = \frac{1}{M} \cdot \sum_i m_i \cdot \vec a_i = \frac{1}{M} \cdot \sum_i \left( \vec F_{\text{ext},i} + \sum_i \vec F_{j,i} \right) = \frac{1}{M} \cdot \left(\sum_i \vec F_{\text{ext},i} + \sum_i \sum_j \vec F_{i,j}\right)\]
...
\[\vec p_{\text{CM}} = \sum_i \vec p_i\]
Pertanto, se
\[\sum \vec F_{\text{ext}} = 0\]
allora si ha che la quantità di moto si conserva, ovvero $\vec p_{\text{CM}}$ costante: tale concetto prende il nome di \textbf{conservazione della qantità di moto}.
...
\[\vec v_{\text{CM}} = \frac{1}{M} \cdot \sum_i m_i \cdot \vec v_i = \frac{m_1 \cdot \vec V_1 + m_2 \cdot \vec V_2}{m_1+m_2} = \frac{\vec P_1 + \vec P_2}{m_1+m_2}=\frac{\vec P_{tot}}{m_{tot}}\]
Pertanto, se ora si considera il sistema di riferimento dato dal centro di massa, il quale si sposta con velocità $\vec v_{\text{CM}}$; ora, con tale nuovo sistema di riferimento, le velocità delle due masse diviene
\[\vec v_1 = \vec V_1 - \vec v_{\text{CM}} \hspace{1em} \text{e} \hspace{1em} \vec v_2 = \vec V_2 - \vec v_{\text{CM}}\]
...
\begin{table}[H]
  \begin{tabularx}{\textwidth}{|P|P|}
    \hline
    \cellcolor{blue!25}\textbf{Conservazione dell'energia} & \cellcolor{green!25}\textbf{Conservazione della quantità di moto}\\
    \hline
...
\[\boxed{V_{1,f} = V_{2,f} = v_{\text{CM}} = \frac{m_1 \cdot V_1 + m_2 \cdot v_2}{m_1 + m_2}}\]
...
\[\vec p_i=\vec p_{cart,i} + \vec p_{ramp,i} = 0 + 0 = \vec 0\]
\[\vec p_f = \vec p_{cart,f} + \vec p_{ramp,f} = \vec v_c \cdot m + \vec v_r \cdot M = 0\]
ma siccome $\vec p_{cart,f} = - \vec p_{r,f}$ si ottiene che $\vec v_{cart} \cdot m = - \vec v_{ramp} \cdot M$.\\
Applicando il principio di conservazione dell'energia meccanica, si ottiene anche che
\[E_i=E_f \longrightarrow E_i=K_{cart,i}+K_{ramp,i} + \mathcal{U}_i = mgh\]
mentre
\[E_f = K_{cart,f} + K_{ramp,f} + \mathcal{U}_f = \frac{1}{2}m v_{cart}^2 + \frac{1}{2}M v_{ramp}^2\]
volendo determinare $v_{ramp}$ si ottiene, applicando la conservazione dell'energia meccanica e il fatto che $\vec v_{cart} = \frac{M \vec v_{ramp}}{m}$
\[mgh=\frac{1}{2}m v_{cart}^2 + \frac{1}{2}M v_{ramp}^2 \longrightarrow mgh=\frac{1}{2}m \left(\frac{M \vec v_{ramp}}{m}\right)^2 + \frac{1}{2}M v_{ramp}^2 \longrightarrow v_{ramp} = \sqrt{\frac{2mh}{\displaystyle{M \cdot \left(1 + \frac{M}{m}\right)}}}\]
...
Per verificare se si conserva l'energia cinetica si consieri:
\[K_i=\frac{1}{2}m \cdot (2v)^2\]
mentre l'energia cinetica finale è data dal centro di massa, ovvero:
\[\vec v_{\text{CM}} = \frac{m v_{1,f} + m v_{2,f}}{2m} = v \longrightarrow \frac{v_{1,f}}{2} = v \longrightarrow \vec v_{1,f} = 2v\]
per cui l'energia cinetica finale è data unicamente dal contributo della prima massa, da cui:
\[K_f=\frac{1}{2}m (2v)^2\]
per cui il moto considerato è elastico.
...
\quotes{Come un metodo per inviare un proiettile verso Ia parte pia alta, persino Ia parte le pia alta, dell'atmosfera terrestre, il razzo a carica multipla del Prof. Goddard 6 un dispositivo practicabile e quindi promettente.
(...) dopo che it razzo ha lasciato Ia nostra aria ed ha iniziato davvero it suo viaggio pia lungo, it suo vuolo non sarebbe ne accelerato ne mantenuto dall'esplosione delle cariche che potrebbe avere ancora. Affermare che lo sarebbe 6 negare una legge fondamentale della dinamica, e solo it dottor Einstein e la sua dozzina prescelta, pochi e capaci, sono autorizzati a farlo.
Che it Prof. Goddard (...) non conosca la relazione tra azione e reazione, e della necessity di avere qualcosa di meglio di un vuoto contro cui reagire — dire questo sarebbe assurdo.}

\vspace{1em}
\noindent
\textbf{Osservazione}: Naturalmente, applicando la seconda legge di Newton generalizza, si ha che
...
in cui, ovviamente, $\vec p_i = m \cdot \vec v_r$, in cui $m$ è la massa totale iniziale del razzo e $\vec v_r$ è la velocità iniziale del razzo. Invece, la quantità di moto finale è $\vec p_f = \left(m - \Delta m\right) \cdot \left(\vec v_r + \\Delta \vec v_r\right) + \Delta m \cdot \left(\vec v_g + \vec v_r\right)$, in quanto il razzo ha perso una quantità $\Delta m$ di gas e, quindi, ha diminuito la propria massa e aumentato la propria velocità; inoltre è necessario anche considerare la quantità di moto della quantità $\Delta m$ di gas che viene espulso dal razzo stesso, la cui velocità, relativa al razzo, è proprio $\vec v_g + \vec v_r$. La semplificazione porta a
...
\[=\frac{1}{\Delta t} \cdot \left(m \Delta \vec v_r + \Delta m \vec v_g - \Delta m \Delta \vec v_r\right)\]
Ma siccome le variazioni considerate sono infinitesimali, allora $\Delta m \Delta \vec v_r \to 0$, da cui
\[\vec p_f = m \cdot \frac{\Delta \vec v_r}{\Delta t} + \vec v_g \cdot \frac{\Delta m}{\Delta t}\]
Considerando un tempo infinitesimale, ossia $\lim_{\Delta t \to 0}$, si ottiene che
...
in cui $\vec v_g \cdot \dfrac{dm}{dt}$ è proprio la \textbf{spinta del razzo}.

\vspace{1em}
\noindent
\textbf{Esempio}: Naturalmente, nello spazio, si ha che $\vec F_{\text{ext}}=0$, per cui l'equazione di cui sopra diviene:
\[m \vec a_r = - \vec v_g \cdot \frac{dm}{dt}\]
Per quanto riguarda la componente orizzontale, riscrivendo l'accelerazione secondo la definizione si ottiene:
...
\[\frac{dv}{dt}=-v_g \frac{d}{dt} \log(m(t)) \longrightarrow v_f-v_i =-v_g \cdot \left[\log(m_f) - \log(m_i)\right]\]
che permette di ottenere
\[\boxed{v_f-v_i=\Delta v=-v_g \log \left(\frac{m_f}{m_i}\right)}\]
che prende il nome di \textbf{equazione di Ciolkovsky}. È facile campire come, essendo
...
\begin{enumerate}
  ...
  \begin{enumerate}
    ...
    \item Riformulare la domanda a parole più chiare,
  \end{enumerate}
  \item \textbf{Descrizione}
  \item \textbf{Pianificazione}
  \begin{enumerate}
    \item Trasferire i concetti in forma matematica
    \item È possibile partire dalla soluzione e lavorare all'indietro
  \end{enumerate}
...
\begin{tabularx}{\textwidth}{@{}|P|@{}}
    \hline
    {\textbf{CORPO RIGIDO}}\\
    \parbox{\linewidth}{Un corpo rigido è un corpo la cui \textbf{forma} e \textbf{dimensione} permangono fissi.\\
    In altre parole, le \textbf{posizioni relative} tra le parti del corpo sono \textbf{fisse}.\vspace{3mm}}\\
    \hline
\end{tabularx}
...
Naturalmente, in una traslazione di un corpo rigido, tutte le parti del corpo subiscono lo \textbf{stesso spostamento} (altrimenti si avrebbe deformazione) e, quindi, presentano la \textbf{medesima velocità}: ciò significa che sono sufficienti solamente $3$ parametri per descrivere una traslazione, ossia $(x,y,z)$.
...
\begin{itemize}
  \item 3 angoli;
  \item l'asse di rotazione più l'angolo.
\end{itemize}
Già approssimando un corpo ad un punto materiale, si era osservato come se un punto compie un moto rotatorio di raggio $r$ e descrivendo un angolo $\theta$, allora, chiamando $s$ la lunghezza dell'arco descritto da un punto compiendo uno spostamente angolare a distanza $r$ dall'asse, si interpreta langolo $\theta$ descritto come
\[\boxed{\theta=\frac{s}{R}}\]
...
\begin{itemize}
  \item \textbf{velocità angolare}: $\dfrac{d\theta}{dt}=\omega$
  \item \textbf{accelerazione angolare}: $\dfrac{d\omega}{dt}=\dfrac{d^2\theta}{dt^2}=\alpha$
\end{itemize}
come nel caso di un accelerazione angolare costante, calcolata come
\[\boxed{\theta(t)=\theta_0+\omega t + \frac{1}{2}\alpha t^2}\]
che ricorda esattamente un la cinematica di un moto uniformemente accelerato (infatti, in questo caso, si una un \textbf{memento angolare costante}).\\

\vspace{1em}
\noindent
\textbf{Osservazione}: Nel caso di un moto rotatorio, è fondamentale considerare l'attrito (che è un attrito statico, non essendoci moto relativo tra le superifici di contatto).\\
...
Partendo dal presuppposto che un corpo rigido è approssimabile tramite il modello di un corpo esteso, ossia un \textbf{assieme di punti materiale con distanze relative fisse}.\\
L'applicazione del centro di massa ad un assieme di punti permette di scrivere la legge:
\[\boxed{\vec F_{\text{tex}} = M \cdot \vec a_{\text{CM}}}\]
che è una proprietà totalemente generale e, quindi, include anche il caso dei corpi rigidi. Ciò permette di affermare che la risultate delle forze esterne, anche se ciascuna presenta un punto di applicazione diverso, si comporta come se fosse un unica forza che agisce sul centro di massa, come se fosse un punto materiale.
...
\vspace{1em}
\noindent
\subsubsection{Corpi rigidi - Energia cinetica}
...
Allora, ricalcolando nuovamente l'energia cinetica totale, si ottiene
...
che permette di ottenere
\[K=\frac{1}{2}M v_{\text{CM}}^2 + \frac{1}{2}\sum_i m_i v_i'^2 + \vec v_{\text{CM}} \cdot \hspace{-4em} \underbrace{\sum_i m_i \cdot \vec v_i}_{0 \text{ perché } \vec p_{tot} \text{ nel centro di massa}}\]
per cui si ottiene il seguente risultato generale (che non dipende dal fatto che si parli di corpi rigidi o meno)
\[\boxed{K_{tot}=K_{\text{CM}}+K_{\text{rel}}}\]

\vspace{1em}
\noindent
\textbf{Osservazione}: Si osservi che, nel caso di un corpo rigido,
\[\boxed{K_{tot}=K_{\text{CM}}+K_{\text{rot}}}\]
in cui $K_{\text{rot}}$ prende il nome di \textbf{energia cinetica di rotazione}. Al fine di determinare tale energia, si consideri una massa $m$ che ruota con velocità angolare $\omega$ descrivendo una circonferenza di raggio $r$. Da ciò segue che l'energia cinetica del corpo è, ovviamente:
\[K=\frac{1}{2}mv^2=\frac{1}{2m(\omega r)^2=\frac{1}{2}m\omega^2r^2}\]
...
prende il nome di \textbf{momento di inerzia}. Ecco che sulla base di tale definizione è possibile definire l'energia cinetica totale di un corpo rigido come
\[\boxed{k_{tot} = K_{\text{CM}} + K_{\text{rot}} = \frac{1}{2}M v_{\text{CM}}^2 + \frac{1}{2}I \omega^2}\]
in cui $I$ è il momento di interzia attorno ad un asse che passa per il centro della massa (sempre ricordando come $v=\omega r$).\\

\vspace{1em}
\noindent
\textbf{Osservazione}: Si osservi che se si considerano due piani inclinati identici, sul primo del quale vi è una massa quadrata $m$ e sul secondo una sfera $m$, allora, se entrambi partono da un'altezza $h$ e si spostano senza attrito, applicando la conservazione dell'energia meccanica, si ottiene:
\begin{enumerate}
  \item $\displaystyle{\frac{1}{2}m v_f^2 = mgh}$
  \item $\displaystyle{\frac{1}{2}m v_f^2 + \frac{1}{2} I \frac{v^2}{r^2} = mgh}$
\end{enumerate}
...
che, in ognuno di talui tre casi, è interpretabile come \textbf{la resistenza di un corpo al cambiamento di velocità angolare} (che è molto simile alla definizione della massa inerziale).\\
...
\[I_{\text{punto}}= m r^2\]
se, invece, si considerano due punti di massa $\frac{m}{2}$ a distanza $r$ dall'asse di rotazione, si ha che
\[I_{2 \text{ punti}} = \frac{m}{2}r^2 + \frac{m}{2}r^2 = mr^2\]
Più in generale, considerando $n$ punti di massa $\frac{m}{n}$ a distanza $r$ dal centro si avrà che
\[I_{n \text{ punti}} = \sum_{i=1}^n \frac{m}{n} r^2 = m r^2\]
e si va al limite si ottiene sempre che
\[I_{\infty \text{ punti}} = \sum_{i=1}^\infty \frac{m}{n} r^2 = m r^2\]
ed è possibile calcolare il momento di inerzia anche per altri corpi:
\begin{itemize}
  \item Sfera piena: $\displaystyle{I=\frac{2}{5} m r^2}$;
  \item Sfera cava: $\displaystyle{I=\frac{2}{3} m r^2}$;
  \item Cilindro: $\displaystyle{I=\frac{2}{5} m r^2}$;
\end{itemize}
...
Inoltre, se l'asse è fisso si ha che $\vert a \parallel \vec \omega$ e se $\vec \alpha$ è costante, allora $\vec \omega = \vec \omega_0 + \vec \alpha \cdot t$.
