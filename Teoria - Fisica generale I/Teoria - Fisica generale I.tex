...
\begin{center}
  28 Marzo 2022
\end{center}
\textbf{Esercizio}: Si consideri una massa $m=500$ g che viene lasciato cadere da un'altezza $h=60$ cm su un ripiano collegato ad una molla di costante elastica $k=120$ v/m. Si determini, allora, la lunghezza $l$ di compressione massima della molla fino a quando l'assieme blocco-piattaforma si ferma.\\
...
\[W_{tot} = \Delta K = \frac{1}{2}m \cdot (v_f^2 - v_i^2)\]
ma essendo la velocità iniziale e finale del blocco nulle, si deduce che il lavoro totale compiuto sul sistema è nullo. Le unice forze che compiono lavoro, in questo caso ipotetico, sono la forza di gravità e la forza elastica di richiamo della molla: ciò significa che il lavoro compiuto da ambedue le forze è uguale, ma opposto, da cui:
...
Naturalmente tale equazione presenterà due soluzioni, ma in questo caso deve chiedere $l>d$:
\[l_{1,2}=\frac{\dfrac{mg}{k}+l \pm \sqrt{4 \cdot \left(\dfrac{mg}{k} + d\right)^2 - 4d^2}}{2}\]
per cui si ottiene che
\[l = \frac{mg}{k} \cdot \left(1+\sqrt{1+\frac{2kd}{mg}}+d\right) = 86.6 \text{ cm}\]

\newpage
\noindent
\subsection{Conservazione dell'energia}
L'energia può essere interpretata come un fluido, la quale si traforma in una forma o in un'altra, ma conservando sempre la propria quantità.
...
\begin{tabularx}{\textwidth}{@{}|P|@{}}
    ...
    \parbox{\linewidth}{Quando si impiega il teorema di conservazione dell'energia, è sempre fondamentale specificare il \textbf{sistema} su cui si sta lavorando, distinguendolo con l'\textbf{ambiente esterno}: tra il sistema e l'ambiente può esserci scambio (o trasferimento) di energia (in ambo le direzioni) oppure no: nel secondo caso si può applicare il \textbf{teorema di conservazione dell'energia}:
    \[\boxed{E_{\text{sistema}}=\underbrace{K+\mathcal{U}}_\text{energia meccanica}+U}\]
    in cui $K$ è l'\textbf{energia cinetica}, $\mathcal{U}$ è l'\textbf{energia potenziale}, mentre $U$ prende il nome di \textbf{energia interna}.\\
    Tuttavia, in \textbf{meccanica}, si ignora totalmente l'energia del sistema, e si preferisce considerare solamente l'energia meccanica $K+\mathcal{U}$.\\
    Se si ha conservazione dell'energia meccanica, si ottiene che
    \[\Delta E_{\text{sistema}}=0=\sum\text{trasferimenti}=\text{lavoro}+\text{calore}\]
    Esempi di sistema possono essere costituiti da
    \begin{itemize}
      \item un corpo solo;
      ...
      \item etc.
    \end{itemize}
    in cui tali sistemi possono essere \textbf{aperti}, \textbf{chiusi} o \textbf{isolati}.\\
    Più limitatamente all'energia meccanica, il teorema di conservazione dell'energia meccanica afferma che
    \[\Delta E_{\text{sistema}}=\Delta K + \Delta \mathcal{U}=W_{\text{sistema}}\]
    ove $W_{\text{sistema}}$ è da intendersi il lavoro totale compiuto dal sistema.
    \vspace{3mm}}\\
    \hline
\end{tabularx}
...
Inoltre, l'energia totale di un sistema può cambiare se una forza esterna compie lavoro sul sistema: lavoro totale compiuto sul sistema è uguale al cambio in energia totale del sistema.
...
ovvero la forza $F$ corrisponde all'opposto della pendenza del grafico forza potenziale spostamento:
...
\noindent
Questo è un sistema che viene definito \textbf{stabile}, in quanto ad ogni spostamento vi è una forza che permette di bilanciare il sistema, portandolo in una condizione di equilibrio.\\
...
\[\frac{d^2 \mathcal{U}}{dx} > 0 \longrightarrow \text{sistema stabile} \hspace{1em} \text{e} \hspace{1em} \frac{d^2 \mathcal{U}}{dx} < 0 \longrightarrow \text{sistema instabile}\]

\vspace{1em}
\noindent
\textbf{Osservazione}: Si osservi che l'energia potenziale gravitazione può essere interpretata come
\[\mathcal{U}(r) = - G \cdot \frac{m_1 \cdot m_2}{r}\]
...
\[F_T = -m \cdot \frac{v_0^2}{L} - mg \cdot \sin(\theta) = -6 mg \cdod \sin(\theta)\]
