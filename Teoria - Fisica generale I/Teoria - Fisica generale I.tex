...
\begin{figure}[H]
  \centering
  \begin{tikzpicture}[scale=1]
    \draw (0,0) node[circ](start){} (-1,2) node[circ](m_1){$m_1$} (2,3) node[circ](m_2){$m_2$};
  \end{tikzpicture}
  \caption{Diagramma a corpo libero di una vettura in rallentamento}
  \label{fig:diagramma_corpo_libero_vettura_rallentamento}
\end{figure}
...
\begin{center}
  21 Marzo 2022
\end{center}
La forza di attrazione tra due corpi è direttamente proporzionale alla massa dei due corpi e inversamente proporzionale alla distanza fra i due (e tale concetto si trasla anche alla teoria della carica, per la \textbf{Forza di Coulomb}).\\
Il \textbf{campo gravitazionale}, inoltre, è una vera e propria forza che si applica indistintamente ad ogni corpo, indipendentemente dalla massa di ogni corpo.\\
La differenza tra massa gravitazionale e massa inserziale è puramente concettuale, anche se nella pratica tali masse coincidono; infine, per quanto concerne i corpi estesi, al fine di considerare la forza attrattiva totale, è sufficiente sommare i contributi di tutte le masse.

\vspace{1em}
\subsection{Leggi di Keplero}
Di seguito si espongono le tre fondamentali leggi di Keplero:
...
\begin{tabularx}{\textwidth}{@{}|P|@{}}
    ...
    \begin{enumerate}
      ...
      \item \emph{La congiungente di un pianeta con il Sole spazza aree uguali in intervalli di tempo uguali}
...
\[\frac{1}{\omega^2} = \frac{1}{G \cdot m_S}\]
...
Si calcoli, allora, la velocità alla quale deve ruotare un satellite (idealmente sulla superficie della Terra). È facile capire che tale velocità si calcola come segue
...
\vspace{1em}
\noindent
\textbf{Osservazione}: Si osservi che dalla terza legge di Keplero si ottiene che
